...
const ELEMENT_EXCESS_SHADOW: Record<string, string> = {
  목: '목 기운이 표면상 강하게 드러나면 성장 욕구가 고집으로 바뀌어 타협이 늦어질 수 있습니다.',
  화: '화 기운이 표면상 강하게 드러나면 감정과 표현이 앞서 관계 피로, 구설, 과열을 만들 수 있습니다.',
  토: '토 기운이 표면상 강하게 드러나면 안정 욕구가 집착과 정체로 변해 변화 대응이 늦어질 수 있습니다.',
  금: '금 기운이 표면상 강하게 드러나면 판단과 기준이 날카로워져 차갑다는 인상을 주기 쉽습니다.',
  수: '수 기운이 표면상 강하게 드러나면 생각과 감정이 깊어지는 대신 실행이 늦고 마음이 가라앉기 쉽습니다.',
};

const ELEMENT_LACK_SHADOW: Record<string, string> = {
  목: '목 기운이 표면상 적게 드러나면 장기 방향, 성장 계획, 꾸준한 확장성이 약해질 수 있습니다.',
  화: '화 기운이 표면상 적게 드러나면 표현력, 자신감, 드러나는 존재감이 약해져 좋은 기회를 숨길 수 있습니다.',
  토: '토 기운이 표면상 적게 드러나면 생활 기반, 신뢰감, 마무리 안정성이 흔들리기 쉽습니다.',
  금: '금 기운이 표면상 적게 드러나면 결단, 정리, 기준 세우기가 약해져 애매한 상태가 길어질 수 있습니다.',
  수: '수 기운이 표면상 적게 드러나면 유연성, 휴식, 깊은 사고가 약해져 무리하게 밀어붙이기 쉽습니다.',
};
...
export function getShadowReading(
  ...
  const pitfalls = [
    ...
    ELEMENT_EXCESS_SHADOW[dominantElement] ?? '표면상 강하게 드러난 기운이 한쪽으로 쏠리면 사고와 행동도 편향될 수 있습니다.',
    ELEMENT_LACK_SHADOW[lackingElement] ?? '표면상 적게 드러난 기운은 평소에는 작게 보이다가 중요한 순간 약점처럼 느껴질 수 있습니다.',
  ];
  ...
  return {
    ...
    pitfalls,
    advice: `표면상 강하게 드러난 ${dominantElement} 기운은 속도를 낮추고, 적게 드러난 ${lackingElement} 기운은 생활 속에서 의식적으로 보완해 보세요. 실제 조후·용신 판단은 함께 보는 편이 좋습니다.`,
  };
}
