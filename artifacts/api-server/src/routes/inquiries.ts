...
router.post("/inquiries", async (req: Request, res: Response) => {
  ...
  const { message, sajuSnapshot, userLabel, inquiryType } = req.body as Record<string, unknown>;
  ...
  const resolvedType = typeof inquiryType === "string" && validTypes.includes(inquiryType) ? inquiryType : "general";
...
router.patch("/inquiries/:id/read", async (req: Request, res: Response) => {
  ...
  await db.update(inquiriesTable).set({ readByUser: true }).where(eq(inquiriesTable.id, id));
  res.json({ ok: true });
});
...
router.get("/admin/inquiries", async (req: Request, res: Response) => {
  ...
  const conditions = status && (status === "pending" || status === "answered")
...
router.get("/admin/inquiries/unread-count", async (req: Request, res: Response) => {
  if (!requireAdmin(req, res)) return;
...
router.patch("/admin/inquiries/:id/read", async (req: Request, res: Response) => {
  ...
  const id = parseInt(String(req.params.id));
  await db.update(inquiriesTable).set({ readByAdmin: true }).where(eq(inquiriesTable.id, id));
  res.json({ ok: true });
});
