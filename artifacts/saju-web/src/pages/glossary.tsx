...
const TERMS: Term[] = [
  ...
  { term:'신강·신약(身強身弱)', hanja:'身強身弱', category:'분석', brief:'일간의 힘이 강한지 약한지',
    detail:'일간(日干)의 힘이 강한지 약한지를 나타냅니다. 일간이 월령(月令)을 얻고(得令) 같은 편 오행인 비겁(比劫)·인성(印星)이 많으면 신강(身強), 그렇지 못하고 일간을 극·설하는 재성·관성·식상이 많으면 신약(身弱)입니다. 신강한 사주는 재(財)·관(官)을 감당할 힘이 있고, 신약한 사주는 인성·비겁의 도움이 필요합니다.' },
  { term:'통근(通根)', hanja:'通根', category:'분석', brief:'천간이 지지에 뿌리를 둔 상태',
    detail:'천간(天干)의 글자가 지지(地支) 속 지장간(地藏干)에 같은 오행의 뿌리를 두고 있는 상태입니다. 통근한 천간은 힘이 강해 합(合)·충(沖)에도 쉽게 흔들리지 않습니다. 반대로 뿌리 없이 떠 있는 천간은 허약하여 외부 변화에 약합니다.' },
  { term:'투출(透出)', hanja:'透出', category:'분석', brief:'지장간이 천간에 드러남',
    detail:'지지(地支) 속에 감춰진 지장간(地藏干)의 오행이 천간(天干)에 같은 글자로 드러나 있는 상태입니다. 투출한 십신은 그 사람의 삶에서 겉으로 뚜렷하게 발현되는 기운으로, 격국(格局)을 정하는 중요한 단서가 됩니다.' },
  { term:'조후(調候)', hanja:'調候', category:'분석', brief:'사주의 한난조습 균형',
    detail:'사주의 한난조습(寒暖燥濕), 즉 차고 더움과 마르고 습함의 균형을 맞추는 이론입니다. 겨울(亥子丑)에 태어나 차가운 사주는 따뜻한 화(火)가, 여름(巳午未)에 태어나 뜨거운 사주는 시원한 수(水)가 조후 용신이 됩니다. 격국과 더불어 용신을 정하는 핵심 기준입니다.' },
  { term:'희신·기신(喜神忌神)', hanja:'喜神忌神', category:'분석', brief:'용신을 돕는 오행과 해치는 오행',
    detail:'용신(用神)을 돕고 살려주는 오행을 희신(喜神), 용신을 해치고 사주의 균형을 깨뜨리는 오행을 기신(忌神)이라 합니다. 대운(大運)·세운(歲運)에서 희신이 들어오면 발전과 성취의 시기, 기신이 들어오면 시련과 정체의 시기로 봅니다.' },
  { term:'납음오행(納音五行)', hanja:'納音五行', category:'분석', brief:'60갑자를 30가지 물상으로 분류',
    detail:'60갑자를 두 간지씩 묶어 해중금·노중화·대림목처럼 30가지 이름과 오행으로 분류하는 보조 체계입니다. 정오행·십신·격국을 바꾸는 핵심 판정이 아니라 각 기둥의 기질을 기억하기 쉬운 물상으로 덧붙여 봅니다. 년주는 초년과 집안, 월주는 사회 환경, 일주는 내면과 관계, 시주는 장기 목표의 결을 보완합니다.' },
  { term:'태원(胎元)', hanja:'胎元', category:'분석', brief:'월주에서 보는 타고난 바탕',
    detail:'태원은 잉태 시기의 기운과 선천적 바탕을 보완해 보는 지표입니다. 명해원은 월주의 천간을 한 자리, 지지를 세 자리 순행해 태원 간지를 구하고, 일간과의 십신·12운성·납음을 함께 풀이합니다. 학파에 따라 다른 산식을 쓰기도 하므로 원국 해석보다 낮은 비중으로 참고합니다.' },
  { term:'명궁(命宮)', hanja:'命宮', category:'분석', brief:'삶의 중심축을 보는 보조 궁위',
    detail:'명궁은 월지와 시지를 바탕으로 삶의 중심 지향과 내적 동력을 보완해 보는 궁위입니다. 명해원은 인(寅)을 1로 둔 월지·시지 지장법과 년상기월법으로 간지를 정합니다. 출생시간이 필요하며 중기·진태양시 보정이나 학파에 따라 결과가 달라질 수 있습니다.' },
  { term:'신궁(身宮)', hanja:'身宮', category:'분석', brief:'후천적 행동 방식을 보는 보조 궁위',
    detail:'신궁은 타고난 생각이 현실 행동과 사회 역할로 어떻게 드러나는지 보완해 보는 궁위입니다. 명해원은 자상기정월 순행 뒤 생시에서 유(酉)까지 역산하는 방식을 쓰고, 년상기월법으로 천간을 정합니다. 명궁과 마찬가지로 출생시간이 필요하며 원국·용신보다 보조적으로 해석합니다.' },
  ...
  { term:'삼합(三合)', hanja:'三合', category:'관계', brief:'세 지지가 모여 강한 오행 국을 이룸',
    detail:'서로 다른 세 지지가 모여 하나의 강력한 오행 기운(局)으로 결합하는 관계입니다. 申子辰은 수국(水局), 亥卯未는 목국(木局), 寅午戌은 화국(火局), 巳酉丑은 금국(金局)을 이룹니다. 가운데 글자(子·卯·午·酉)가 왕지(旺支)로 중심이 되며, 삼합이 이뤄지면 해당 오행의 힘이 매우 강해집니다.' },
  { term:'육합(六合)', hanja:'六合', category:'관계', brief:'두 지지가 짝을 이뤄 결합',
    detail:'이웃한 두 지지가 짝을 이뤄 결합하는 관계입니다. 子丑合(토), 寅亥合(목), 卯戌合(화), 辰酉合(금), 巳申合(수), 午未合의 여섯 쌍이 있습니다. 육합은 친화·결속·인연을 의미하며, 사주에 합이 많으면 정이 많고 사람과의 인연이 깊은 편입니다.' },
  { term:'방합(方合)', hanja:'方合', category:'관계', brief:'같은 방위·계절의 세 지지가 모임',
    detail:'같은 방위와 계절을 공유하는 세 지지가 모여 강한 오행을 이루는 관계입니다. 寅卯辰은 동방 목국(봄), 巳午未는 남방 화국(여름), 申酉戌은 서방 금국(가을), 亥子丑은 북방 수국(겨울)입니다. 삼합보다 순수하고 강한 계절의 기운을 형성합니다.' },
  { term:'파(破)', hanja:'破', category:'관계', brief:'두 지지가 서로 깨뜨리는 관계',
    detail:'지지 육파(六破)는 子酉·午卯·巳申·寅亥·辰丑·戌未의 여섯 쌍으로, 서로의 기운을 깨뜨리고 분리시키는 관계입니다. 충(沖)보다는 작용이 약하지만 관계의 균열, 계획의 차질, 헤어짐을 의미합니다.' },
  { term:'해(害)', hanja:'害', category:'관계', brief:'가까운 사이에서 서로 해치는 관계',
    detail:'지지 육해(六害)는 子未·丑午·寅巳·卯辰·申亥·酉戌의 여섯 쌍으로, 천(穿)이라고도 합니다. 가까운 사이에서 생기는 시기·방해·배신·질병을 의미하며, 육친(六親) 사이의 보이지 않는 갈등으로 나타나기 쉽습니다.' },
  { term:'원진(怨嗔)', hanja:'怨嗔', category:'관계', brief:'까닭 없이 서로 미워하는 관계',
    detail:'원진(怨嗔)은 子未·丑午·寅酉·卯申·辰亥·巳戌의 여섯 쌍으로, 뚜렷한 이유 없이 서로 미워하고 원망하는 관계입니다. 부부·가족 사이에 원진이 있으면 사소한 일로 다투기 쉽고, 애증이 엇갈리는 인연이 되기 쉽습니다.' },
  ...
  { term:'사주팔자(四柱八字)', hanja:'四柱八字', category:'기본', brief:'태어난 연·월·일·시의 여덟 글자',
    detail:'태어난 연(年)·월(月)·일(日)·시(時)를 각각 천간과 지지로 나타낸 네 기둥(四柱), 총 여덟 글자(八字)를 말합니다. 년주는 조상·초년, 월주는 부모·사회·청년기, 일주는 자신·배우자, 시주는 자녀·말년을 상징합니다. 사주 해석의 전체 틀이 됩니다.' },
  { term:'음양(陰陽)', hanja:'陰陽', category:'기본', brief:'만물을 이루는 두 기운',
    detail:'음(陰)과 양(陽)은 우주 만물을 이루는, 상반되면서도 서로 보완하는 두 기운입니다. 천간·지지·오행 모두 음양으로 나뉘며(예: 甲은 양목, 乙은 음목), 사주 해석의 가장 기본이 되는 개념입니다. 음양의 균형이 잘 잡힌 사주를 조화로운 사주로 봅니다.' },
  { term:'지장간(地藏干)', hanja:'地藏干', category:'기본', brief:'지지 속에 숨어 있는 천간',
    detail:'각 지지(地支) 속에 감춰져 있는 천간(天干)을 말합니다. 보통 여기(餘氣)·중기(中氣)·정기(正氣)의 순서로 2~3개의 천간이 들어 있습니다. 예를 들어 인(寅) 속에는 戊·丙·甲이 들어 있습니다. 지장간은 사주의 숨은 기운과 통근(通根)·격국(格局)을 판단하는 핵심 자료입니다.' },
];
