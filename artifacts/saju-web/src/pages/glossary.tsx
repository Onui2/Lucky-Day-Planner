...
const TERMS: Term[] = [
  ...
  { term:'득령(得令)', hanja:'得令', category:'분석', brief:'일간이 태어난 계절의 도움을 얻음',
    detail:'월지의 계절 기운이 일간 또는 일간을 생하는 인성을 도와주는 상태입니다. 신강·신약 판단에서 비중이 크지만, 득령 하나만으로 일간이 강하다고 확정하지 않고 통근과 원국 전체의 지원·소모를 함께 봅니다.' },
  { term:'득지(得地)', hanja:'得地', category:'분석', brief:'일간이 지지에 안정된 뿌리를 얻음',
    detail:'일간이 일지나 다른 지지의 지장간에서 같은 오행 또는 인성의 뿌리를 얻은 상태입니다. 뿌리의 위치와 정기·중기·여기 여부에 따라 힘이 다르며, 가까운 일지의 뿌리는 실제 생활의 버팀목으로 비교적 직접 작동합니다.' },
  { term:'득세(得勢)', hanja:'得勢', category:'분석', brief:'원국 전체에서 같은 편의 세력을 얻음',
    detail:'비겁과 인성이 천간·지지에 고르게 있어 일간을 돕는 세력이 충분한 상태입니다. 글자 수만 세지 않고 월지 비중, 지장간 가중치, 통근·투출을 함께 봐야 하며, 득세가 지나치면 오히려 설기와 제어가 필요한 신강 구조가 됩니다.' },
  { term:'억부용신(抑扶用神)', hanja:'抑扶用神', category:'분석', brief:'강한 것은 누르고 약한 것은 돕는 용신',
    detail:'일간의 세력이 지나치게 강하면 설기·재성·관성으로 누르고, 약하면 비겁·인성으로 돕는 균형법입니다. 신강·신약 판정이 먼저 정확해야 하며 조후·격국과 결론이 다를 때는 어느 기준이 실제 원국을 더 잘 설명하는지 교차 검토합니다.' },
  { term:'통관용신(通關用神)', hanja:'通關用神', category:'분석', brief:'충돌하는 두 오행 사이를 이어주는 용신',
    detail:'강한 두 오행이 서로 극해 흐름이 막힐 때 두 기운 사이를 상생으로 연결하는 오행을 말합니다. 예를 들어 금과 목의 대치에는 수가 금생수·수생목으로 통로가 될 수 있습니다. 실제 세력이 약한 대치라면 통관을 주용신으로 과대평가하지 않습니다.' },
  { term:'병약용신(病藥用神)', hanja:'病藥用神', category:'분석', brief:'원국의 핵심 병을 다스리는 약의 오행',
    detail:'사주에서 균형을 가장 크게 깨뜨리는 과다·혼잡·막힘을 병으로 보고 이를 직접 조절하는 오행을 약으로 정합니다. 단순히 가장 부족한 오행을 채우는 방식과 다르며, 병이 명확하지 않으면 보조 판단으로만 사용합니다.' },
  { term:'합화(合化)', hanja:'合化', category:'관계', brief:'천간합이 특정 오행으로 실제 변화함',
    detail:'갑기합토·을경합금·병신합수·정임합목·무계합화처럼 두 천간이 합한 뒤 목표 오행으로 변하는 현상입니다. 합이 있다고 곧바로 합화가 되는 것은 아니며 월령의 지원, 목표 오행의 통근, 다른 천간의 충·방해가 적은지를 확인해야 합니다.' },
  { term:'교운기(交運期)', hanja:'交運期', category:'기본', brief:'이전 대운과 다음 대운이 교차하는 때',
    detail:'10년 대운이 바뀌는 전후의 적응 구간입니다. 이전 흐름의 마무리와 새 흐름의 주제가 함께 나타날 수 있어 직업·관계·생활 리듬이 흔들리기 쉽습니다. 특정 하루로 단정하기보다 전후 약 1년의 반복 변화를 기록해 확인하는 편이 실용적입니다.' },
  { term:'진태양시(眞太陽時)', hanja:'眞太陽時', category:'기본', brief:'출생지 경도와 균시차를 반영한 태양시',
    detail:'행정 표준시가 아니라 출생지에서 실제 태양이 남중하는 시각을 기준으로 보정한 시간입니다. 표준 자오선과 출생지 경도 차이, 계절별 균시차, 당시의 역사적 시간대·서머타임을 반영합니다. 시주 경계에 가까울수록 결과에 영향을 줄 수 있습니다.' },
  { term:'야자시(夜子時)', hanja:'夜子時', category:'기본', brief:'밤 11시부터 다음 날 일주로 보는 기준',
    detail:'자시를 밤 11시~새벽 1시로 보고, 그중 밤 11시 이후를 다음 날의 일주로 넘기는 학파의 기준입니다. 다른 학파는 자정 0시에 일주를 바꾸므로 23시대 출생자는 두 기준을 비교해야 합니다. 시주는 보정된 현지 시각을 기준으로 계산합니다.' },
  { term:'육친(六親)', hanja:'六親', category:'분석', brief:'십신으로 읽는 가족과 사회적 관계 역할',
    detail:'비겁·식상·재성·관성·인성을 부모, 형제·동료, 배우자, 자녀, 조직 역할과 연결해 해석하는 체계입니다. 성별에 따라 배우자·자녀를 보는 십신이 달라질 수 있으며, 특정 가족의 실제 성격을 단정하기보다 관계에서 반복되는 역할과 기대를 읽는 데 씁니다.' },
  { term:'궁성(宮星)', hanja:'宮星', category:'분석', brief:'네 기둥의 자리와 십신을 함께 보는 법',
    detail:'년주·월주·일주·시주의 자리 의미인 궁과, 그 자리에 놓인 십신·오행인 성을 함께 해석하는 방법입니다. 년주는 집안·초년, 월주는 부모·사회, 일지는 배우자·생활, 시주는 자녀·후반기를 주로 보며 합충형해가 해당 관계 영역을 어떻게 움직이는지도 확인합니다.' },
];
