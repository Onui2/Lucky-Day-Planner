    ...
    <>
      ...
      >
        ...
        {!open && data && !hasUnlimitedAccess && data.remaining > 0 && (
          <span className="ml-1 rounded-full bg-foreground/10 px-1.5 py-0.5 text-[10px]">
            {data.remaining}
          </span>
        )}
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-28 right-4 z-50 w-[min(380px,calc(100vw-2rem))] flex flex-col rounded-2xl border border-violet-400/30 bg-white/95 shadow-2xl shadow-violet-900/15 backdrop-blur-xl overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-foreground/10 bg-violet-500/10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-violet-700">AI 사주 상담</span>
              </div>
              <div className="flex items-center gap-3">
                {isAuthenticated && (
                  <span className="text-[11px] text-violet-700/80">{isLoading ? "..." : remainingLabel}</span>
                )}
            ...
            {/* 입력 영역 */}
            {isAuthenticated && (
              <div className="border-t border-foreground/10 bg-foreground/5 px-3 py-2.5 flex gap-2 items-end">
                <textarea
                  value={question}
